...
mod tests {
    ...
    #[test]
    fn mem_pool_4() -> anyhow::Result<()> {
        let pool = MemPool::<4>::new(40);

        let result = pool.alloc(Point { x: 56., y: 69. });
        assert_eq!(result.is_err(), true);
    ...
    #[test]
    fn mem_pool_2() -> anyhow::Result<()> {
        let pool = MemPool::<2>::new(40);
